...
const DOMAINS: Record<NewsDomain, DomainDef> = {
  'ai-tech': {
    ...
    feeds: [
      'https://news.google.com/rss/search?q=artificial+intelligence+OR+AI+OR+LLM+OR+OpenAI+OR+NVIDIA+when:1d&hl=en-US&gl=US&ceid=US:en',
      'https://news.google.com/rss/search?q=technology+startup+OR+chip+OR+robot+when:1d&hl=en-US&gl=US&ceid=US:en',
    ],
  },
  'finance': {
    label: '金融经济',
    ...
    feeds: [
      'https://news.google.com/rss/search?q=economy+OR+stock+market+OR+federal+reserve+OR+inflation+when:1d&hl=en-US&gl=US&ceid=US:en',
      'https://news.google.com/rss/search?q=cryptocurrency+OR+trade+war+OR+GDP+when:1d&hl=en-US&gl=US&ceid=US:en',
    ],
  },
  'geopolitics': {
    label: '国际政治',
    ...
    feeds: [
      'https://news.google.com/rss/search?q=geopolitics+OR+war+OR+diplomacy+OR+sanctions+OR+NATO+when:1d&hl=en-US&gl=US&ceid=US:en',
      'https://news.google.com/rss/search?q=United+Nations+OR+conflict+OR+foreign+policy+when:1d&hl=en-US&gl=US&ceid=US:en',
    ],
  },
  'automotive': {
    label: '汽车',
    ...
    feeds: [
      'https://news.google.com/rss/search?q=electric+vehicle+OR+Tesla+OR+BYD+OR+autonomous+driving+when:1d&hl=en-US&gl=US&ceid=US:en',
      'https://news.google.com/rss/search?q=automotive+industry+OR+car+sales+OR+EV+battery+when:1d&hl=en-US&gl=US&ceid=US:en',
    ],
  },
};
...
export class TopicMatcher {
  ...
  constructor() {
    this.parser = new Parser({ headers: { 'User-Agent': UA, Accept: 'application/rss+xml, application/xml, text/xml, */*' }, timeout: FETCH_TIMEOUT });
  }
  ...
  private async extractArticle(url: string): Promise<string> {
    ...
    } catch (err) { console.warn(`  ⚠️ 正文提取失败: ${url} - ${err instanceof Error ? err.message : err}`); return ''; }
  }
    ...
    for (const domain of Object.keys(DOMAINS) as NewsDomain[]) {
      ...
      for (const article of selected) {
        ...
        allItems.push({
          ...
          englishArticle: { articleId: randomUUID(), sourceId: 'google-news', sourceName: article.source, title: article.title, summary: enContent.slice(0, 200), content: enContent, url: article.url, publishedAt: article.publishedAt },
