...
- **Briefing_Generator（简报生成器）**：负责从中文互联网新闻源抓取、筛选并生成每日简报的后端服务组件
- **News_Entry（新闻条目）**：简报中的单条新闻，包含中文标题、英文翻译标题、摘要、来源和领域分类
- **Study_Session（研习会话）**：用户针对某条新闻发起的一次双语对照阅读活动，包含中文原文和用户提供的英文原文
- **Content_Extractor（内容提取器）**：负责访问从中文互联网抓取的中文报道和用户提供的英文报道链接并智能提取纯正文内容的后端服务组件
...
- **Domain（领域）**：新闻和术语的分类维度，固定为四个领域：AI科技、国际金融经济、国际政治、汽车
